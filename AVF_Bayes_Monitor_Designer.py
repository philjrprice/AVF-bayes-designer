...
st.markdown("Updated: Added Dynamic Futility Slider while maintaining independent Efficacy and Safety Priors.")
...
# NEW FEATURE: Futility slider added to your sidebar
fut_conf = st.sidebar.slider("Futility Stop Threshold", 0.01, 0.20, 0.05, 
    help="Stop if Prob(Efficacy > Hurdle) < this level. Higher = more aggressive early stopping.")
...
def run_fast_batch(sims, max_n, p_eff, p_sae, hurdle, e_conf, limit, cohort_sz, s_conf, f_conf, p_a, p_b, s_a, s_b):
    ...
    for n in range(cohort_sz, max_n + 1, cohort_sz):
        ...
        # ENGINE UPDATE: Logic now uses the futility slider value
