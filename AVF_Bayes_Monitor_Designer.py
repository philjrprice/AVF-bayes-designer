...
# Basic rates
p0 = st.sidebar.number_input("Null response rate (p0)", 0.0, 1.0, 0.20, 0.01)
p1 = st.sidebar.number_input("Target response rate (p1)", 0.0, 1.0, 0.40, 0.01)
...
eff_prob_interim = colT1.number_input("Efficacy interim Pr[p>p0]", 0.5, 1.0, 0.99, 0.01)
eff_prob_final   = colT2.number_input("Efficacy final Pr[p>p0]",   0.5, 1.0, 0.975, 0.005)
colT3, colT4 = st.sidebar.columns(2)
p_tox_star   = colT3.number_input("Max acceptable tox (p_tox*)", 0.0, 1.0, 0.30, 0.01)
saf_prob_cut = colT4.number_input("Safety cutoff prob.", 0.5, 1.0, 0.90, 0.01)

fut_prob_cut = st.sidebar.number_input("Futility cutoff (predictive)", 0.0, 0.5, 0.05, 0.01)
...
max_type1 = colO1.number_input("Max Type I", 0.01, 0.5, 0.10, 0.01)
min_power = colO2.number_input("Min Power", 0.5, 1.0, 0.80, 0.01)
flex_tol  = colO3.number_input("Tolerance (±)", 0.0, 0.2, 0.02, 0.01)

# Simulation settings
st.sidebar.subheader("Simulation Settings")
colS1, colS2 = st.sidebar.columns(2)
rand_seed = int(colS1.number_input("Random seed", min_value=0, value=12345, step=1))
engine = colS2.selectbox("Engine", ["FAST", "ACCURATE"], index=0)
...
with main_tabs[0]:
    ...
    N_min = int(c1.number_input("N min", min_value=5, value=40, step=1))
    N_max = int(c2.number_input("N max", min_value=N_min, value=80, step=1))
    N_step = int(c3.number_input("Step", min_value=1, value=5, step=1))
    c4, c5 = st.columns(2)
    sims_fast = int(c4.number_input("Sims per N", min_value=500, value=3000, step=500))
    ptox_eval = float(c5.number_input("True toxicity (acceptable)", 0.0, 1.0, max(0.01, min(p_tox_star - 0.05, 0.99)), 0.01))
...
with main_tabs[1]:
    ...
    Ns_text = st.text_input("N values (comma)", value="40, 50, 60, 70, 80")
    sims_cmp = int(st.number_input("Sims per N", min_value=1000, value=5000, step=1000))
    ptox_eval = float(st.number_input("True toxicity (acceptable)", 0.0, 1.0, max(0.01, min(p_tox_star - 0.05, 0.99)), 0.01))
...
with main_tabs[2]:
    ...
    ptox_deep = float(st.number_input("True toxicity (acceptable)", 0.0, 1.0, max(0.01, min(p_tox_star - 0.05, 0.99)), 0.01))
...
with main_tabs[3]:
    ...
    c1, c2 = st.columns(2)
    p_eff_min = float(c1.number_input("Efficacy min", 0.0, 1.0, max(0.0, p0-0.1), 0.01))
    p_eff_max = float(c1.number_input("Efficacy max", p_eff_min, 1.0, min(1.0, p1+0.2), 0.01))
    p_eff_step= float(c1.number_input("Efficacy step", 0.01, 0.5, 0.05, 0.01))
    p_tox_min = float(c2.number_input("Toxicity min", 0.0, 1.0, max(0.0, p_tox_star-0.2), 0.01))
    p_tox_max = float(c2.number_input("Toxicity max", p_tox_min, 1.0, min(1.0, p_tox_star+0.2), 0.01))
    p_tox_step= float(c2.number_input("Toxicity step", 0.01, 0.5, 0.05, 0.01))
...
with main_tabs[4]:
    ...
    N_sel = int(st.number_input("Final N (selected)", min_value=5, value=60, step=1))
    ...
    if st.button("Compile & show decision boundaries"):
    ...
    if st.button("Set as Selected Design"):
        selection = {
    ...
    if st.button("Build export zip"):
        try:
            ...
            st.download_button(label="Download Export (.zip)", data=zip_bytes, file_name="bayesian_single_arm_design_export.zip", mime="application/zip")
