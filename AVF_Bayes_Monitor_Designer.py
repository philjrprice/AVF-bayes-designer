...
# v3.1.5e: Adds OC/ESS curves (fixed q), expands summary look-points (incl. final), removes stray groupby(function=...) line
...
try:
    ...
    _HAS_PLOTLY = False
SCHEMA_VERSION = "v3_1_5e_no_colQ3"
...
st.set_page_config(page_title="Bayesian Single‑Arm Designer (Binary) — v3.1.5e", layout="wide")
st.title("Bayesian Single‑Arm Monitored Study Designer (Binary Endpoint) — v3.1.5e")
...
def make_design_pdf(design: Dict, deep_results: Optional[Dict], compare_df: Optional[pd.DataFrame]) -> bytes:
    ...
    def line(text: str, dy: float = 0.6*cm, bold: bool = False):
        ...
        y -= dy
    line("Bayesian Single-Arm Design – Summary (v3.1.5e)", bold=True)
    line("")
...
st.download_button("Download JSON bundle", data=json_bytes, file_name="design_and_results_v3_1_5e.json", mime="application/json")
...
st.download_button("Download ZIP (CSVs)", data=buf.getvalue(), file_name="design_results_v3_1_5e_csv.zip", mime="application/zip")
...
if st.button("Download protocol‑ready PDF", key='download_pdf'):
    ...
    st.download_button("Click to download PDF", data=pdf_bytes, file_name="design_summary_v3_1_5e.pdf", mime="application/pdf")
